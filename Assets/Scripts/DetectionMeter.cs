...
public class DetectionMeter : MonoBehaviour
{
    ...
    private bool isActivelyDetected = false;
    private float pulseTimer = 0f;

    private void Start()
    {
        // Always start empty regardless of serialized value
        detectionLevel = 0f;
    }
    ...
    /// <summary>Called by GuardAI each frame — true when guard currently sees/detects player.</summary>
    public void SetActiveDetection(bool active)
    {
        isActivelyDetected = active;
    ...
    private void UpdateAlertIcon()
    {
        ...
        if (isActivelyDetected)
        {
            ...
            float intensity = Mathf.Clamp01(detectionLevel / 1f);
            alertIcon.color = Color.Lerp(new Color(0.8f, 0.1f, 0.1f, 1f), new Color(1f, 0f, 0f, 1f), intensity);

            pulseTimer += Time.deltaTime * 4f;
            float pulse = Mathf.Lerp(0.75f, 1.1f, (Mathf.Sin(pulseTimer) + 1f) * 0.5f);
    ...
    public void IncreaseDetection(float amount) => detectionLevel = Mathf.Clamp01(detectionLevel + amount);
    public void DecreaseDetection(float amount) => detectionLevel = Mathf.Clamp01(detectionLevel - amount);
    public void ResetDetection()                => detectionLevel = 0f;
}
